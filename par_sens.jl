...
# Parameter file to perform sensitivity analyses fpr Ea and ctag_max as a function of temperature and at two nutrient regimes. 
# We started with the same parameterization as for the "warm-adapted diatom" with the exception of:
# Ktr = 10
# Ea = from 0.5 to 1.0 (10 values)
# ctag_max = from 6e6 to 2.4e7 (10 values)
# total of phenotypes simulated: 100
...
K = Dict("tr" => 10.0, "ri" => 10^4, "lb" => 10^4, "p" => 10,
...
change_Ea = round.(collect(range(0.55, stop=1.05, length=10)); digits=2)    # eV, thermal dependency of metabolic reactions
change_ctag_max = collect(range(6e6, stop=2.4e7, length=10))                # maximum number of lipid molecules / um3, heat-mitigation capacity (alpha)
